...
class Strategy(metaclass=ABCMeta):
    ...
    def __str__(self):
        params = ",".join(
            f"{i[0]}={i[1]}"
            for i in zip(
                self._params.keys(), map(_as_str, self._params.values())
            )
        )
        if params:
            params = "(" + params + ")"
...
class Trade:
    ...
    def __init__(
        self, broker: "_Broker", size: int, entry_price: float, entry_bar
    ):
        self.__broker = broker
        self.__size = size
...
class _Broker:
    ...
    def new_order(
        ...
        *,
        ...
        if is_long:
            if (
                not (sl or -np.inf)
                < (limit or stop or adjusted_price)
                < (tp or np.inf)
            ):
                raise ValueError(
                    "Long orders require: "
        ...
        for order in list(self.orders):  # type: Order
            ...
            if order.parent_trade:
                ...
                if trade in self.trades:
                    ...
                    assert (
                        order.size != -_prev_size or trade not in self.trades
                    )
    ...
    def _reduce_trade(
        self, trade: Trade, price: float, size: float, time_index: int
    ):
...
class Backtest:
    ...
    def __init__(
        ...
        *,
        ...
        close_order_at_the_end: bool = True,
    ):
        """
        ...
        if not isinstance(commission, Number):
            raise TypeError(
                "`commission` must be a float value, percent of "
                "entry order price"
            )
        ...
        if (
            ...
            (
                ...
                and (data.index > pd.Timestamp("1975").timestamp()).mean()
                > 0.8
            )
        ):
            try:
        ...
        self.close_order_at_the_end = close_order_at_the_end
    ...
    def run(self, **kwargs) -> pd.Series:
        ...
        with np.errstate(invalid="ignore"):
            ...
            for i in range(start, len(self._data)):
                ...
                if self.close_order_at_the_end:
            ...
            equity = (
                pd.Series(broker._equity).bfill().fillna(broker._cash).values
            )
            self._equity = equity

            self._closed_trades = broker.closed_trades
            self._trades = sorted(
                broker.closed_trades + broker.trades,
            ...
            try:
                ...
                if mp.get_start_method(allow_none=False) == "fork":
                    with ProcessPoolExecutor() as executor:
                        futures = [
                            executor.submit(
                                Backtest._mp_task, backtest_uuid, i
                            )
    ...
    def _mp_task(backtest_uuid, batch_index):
        bt, param_batches, maximize_func = Backtest._mp_backtests[
            backtest_uuid
        ]
        return batch_index, [
            maximize_func(stats) if stats["# Trades"] else np.nan
            for stats in (
                bt.run(**params) for params in param_batches[batch_index]
            )
        ]
